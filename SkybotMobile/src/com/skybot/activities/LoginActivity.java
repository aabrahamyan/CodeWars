		...
		if ((username.getText().toString()).equals(password.getText()
			...
			Map<String, String> job_params = creator
					.createAppropriateMapRequest(Constants.DATE, system_Time,
							Constants.RESULTS, "300", Constants.SORT, "name",
							Constants.DIRECTION, "ASC", Constants.TAG, "",
							Constants.TAG_MATCH_ANY, "false", Constants.START,
							"0", Constants.LIMIT, "300");
			...
			// ------------------------ Job Details request
			Map<String, String> job_details = creator
					.createAppropriateMapRequest(Constants.DATE, system_Time,
							Constants.LIST, "all");
			String urlWithParams = reqHelper.constructGetRequestString(
					job_details, Constants.SERVER_URL);
